...
**2026-09-16 basic editing slice:** [STUDIO-03](./studio-editing.md) implements node addition/removal, atomic incident cleanup, connection replacement/disconnection and every existing parameter kind. STUDIO-01 authored transport and initial-template prerequisites are resolved: Rust validates original bytes, numeric lexemes and complete JSON fields are retained, then Rust validates candidate bytes. No engine package update is needed. Only the current draft and original checkpoint are retained; undo/redo, binding editing, formal material saving and STUDIO-05 remain unimplemented. [Evidence](./evidence/studio-editing/README.md) separates local isolated and remote checks.
