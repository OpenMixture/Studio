...
**Active follow-up plan (2026-09-16):** [Studio MVP stabilization and external trial](./studio-alpha.md) assigns the next product work and engine dependencies. This document retains the completed MVP scope and architecture rules; its original batches are historical, not a new feature backlog.
...
**2026-09-16 read-only slice:** The [source graph view](./studio-graph.md) implements STUDIO-02 and its read-only STUDIO-01 prerequisites at `/player/studio.html`. Rust validates source bytes before a display projection preserving numeric text; only original `.mix` bytes and separate layout are saved. STUDIO-01 authored serialization/history/new-document work is deferred until before STUDIO-03/04 and is not marked fully complete. This sequencing refinement delivers the requested read-only view without changing the engine archive. [Evidence](./evidence/studio-graph/README.md) distinguishes local, isolated and remote checks; At that checkpoint, STUDIO-03 through STUDIO-05 were unimplemented.

**2026-09-16 basic editing slice:** [STUDIO-03](./studio-editing.md) implements node addition/removal, atomic incident cleanup, connection replacement/disconnection and every existing parameter kind. STUDIO-01 authored transport and initial-template prerequisites are resolved: Rust validates original bytes, numeric lexemes and complete JSON fields are retained, then Rust validates candidate bytes. No engine package update is needed. At that checkpoint only the current draft and original checkpoint were retained; history and saving followed in STUDIO-04. [Evidence](./evidence/studio-editing/README.md) separates local isolated and remote checks.

**2026-09-16 history/save slice:** [STUDIO-04](./studio-save.md) completes bounded history, public bindings, independent save checkpoints and failure recovery. STUDIO-01 prerequisites are complete. At that checkpoint, cross-consumer STUDIO-05 remained pending. [Evidence](./evidence/studio-save/README.md) separates measured checks from integration.

**2026-09-16 cross-consumer acceptance:** [STUDIO-05 evidence](./evidence/studio-qualification/README.md) closes all five MVP gates for the recorded environment: actual UI saves, seven native/Player 1K cases, 28 channel comparisons, isolation, lifecycle and production deployment. No tolerance or native golden changed. Broader matrices, publishing, hosting and M6 remain separate decisions.
...
The MVP covers the existing node catalog and material channels, a desktop graph workspace with keyboard-accessible editing controls, selection, pan/zoom, node movement, diagnostics and unsaved-change feedback. It preserves the existing Player entry and workflow. The Studio entry is `/player/studio.html`, sharing the existing static base with Player.
...
- Keep positions, viewport and selection outside `.mix`. An optional `.mix.layout.json` sidecar is product-owned, versioned and associated with document identity and node IDs. Missing layout uses defaults; malformed or mismatched layout preserves current positions and cannot block material validation/rendering. Moving nodes changes layout dirtiness, not material bytes.
