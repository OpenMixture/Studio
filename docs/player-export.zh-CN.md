...
macOS 隔离步骤禁止访问两个原始仓库，并从 PATH 移除 Rust。保留干净已测修订、未变更归档摘要、运行时上下文、解码测量及选定 PNG／截图。这些检查验证有界 Player 流程；M5-05 原生／浏览器 1K 比较、更广兼容性、压力与正式浏览器 CI 仍属独立门槛。不包含包发布、网站部署或 Studio 编辑。
