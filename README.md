...
**Next phase:** [Studio MVP stabilization and external trial](./docs/studio-alpha.md) assigns branch protection, ordinary-browser qualification, candidate-runtime upgrade acceptance, focused entry cleanup and user trials to this repository. Current-candidate package CI and npm release remain engine-owned dependencies. [P1 evidence](./docs/evidence/alpha-p1/README.md) records enforced main protection and the passing ordinary Windows/Chrome workflow for the historical archive. [The exact candidate upgrade](./docs/evidence/runtime-alpha04/README.md) now passes seven-case/28-channel comparisons and 52 contracts each on Windows and isolated Linux, plus ordinary Chrome save/Player/export; the earlier failed candidate remains a historical record. Advanced editor features and M6 remain deferred.
...
The Studio entry is `/player/studio.html`, with node addition/removal, connect/disconnect, parameter editing, diagnostics and shared previews. Save edited material and layout separately, undo/redo changes and author public bindings. Original downloads remain unchanged. See [Studio saving](./docs/studio-save.md) for recovery and checkpoint boundaries.
