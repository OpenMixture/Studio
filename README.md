...
An independent product repository for OpenMixture. The first deliverable is a small **Player** that opens `.mix` source, explicitly initializes WebGPU, and previews the engine's returned baseColor pixels. Studio graph authoring comes later.
...
Open the local `/player/` URL printed by Vite. Choose the included checker or a `.mix` file, optionally validate it, select dimensions, click **Initialize WebGPU**, then **Render**. **Dispose GPU** releases the renderer while retaining the displayed preview. Validation loads WASM but does not acquire a GPU. File bytes are passed directly to Rust; the source display is never parsed and reserialized for rendering.
...
The `Product checks` workflow runs a clean npm install, public type checks and the production build. It does **not** certify browser GPU execution.
