import type { ChannelId, GpuRuntime, ParameterValue, RenderedChannel, RenderResult, RuntimeModule, ValidationResult } from '@openmixture/runtime';
...
export interface AuthoringHooks {
  invalidate(): void;
  changed(bytes: Uint8Array | undefined, result: ValidationResult): void;
  create(bytes: Uint8Array): void;
}
export interface AuthoringSession {
  prepare(module: RuntimeModule, bytes: Uint8Array, name: string): Promise<() => void>;
  confirmReplace(): boolean;
  clear(): void;
  load(module: RuntimeModule, bytes: Uint8Array, name: string, fresh: boolean): void;
}

export function startPreview(createAuthoring?: (hooks: AuthoringHooks) => AuthoringSession): void {
  ...
  const authoring = createAuthoring?.({
    ...
    changed(bytes, result) {
      source = bytes;
      element('source-text').textContent = bytes ? new TextDecoder().decode(bytes) : 'Repair the draft fields to produce source bytes.';
  ...
  async function loadSource(read: () => Promise<Uint8Array>, name: string, initial = false, fresh = false): Promise<void> {
    if (authoring) {
      ...
      try {
        ...
        const commitGraph = await authoring.prepare(module, bytes, name);
        if (generation !== sourceGeneration) return;
        if (!authoring.confirmReplace()) { samples.value = currentSample; return; }
        scheduler?.invalidate(); authoring.clear(); commitGraph();
        ...
        authoring.load(module, bytes, name, fresh); markStale('source changed');
