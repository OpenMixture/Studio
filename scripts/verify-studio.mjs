...
try {
 ...
 const contract=await browser.newPage();await contract.goto('http://127.0.0.1:4173/player/tests/contract.html');
 ...
 const receipt={schemaVersion:1,manifestSha256:sha(raw),archiveSha256:manifest.archiveSha256,lockSha256:sha(await readFile('package-lock.json')),productRevision:manifest.productRevision,browser:browser.version(),os:platform(),osRelease:release(),arch:arch(),node:process.version,args,build,cases:[]};
 await page.goto('http://127.0.0.1:4173/player/');await expect(page.locator('#status')).toContainText('Checker loaded');
