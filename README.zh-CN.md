...
这是 OpenMixture 的独立产品仓库。首个交付物是小型 **Player**：打开 `.mix` 源文件，显式初始化 WebGPU，并预览引擎返回的 baseColor 像素。Studio 节点编辑功能后续再做。
...
打开 Vite 输出的本地 `/player/` 地址。选择内置棋盘格或 `.mix` 文件，可先验证，设置尺寸后点击 **Initialize WebGPU**，再点击 **Render**。**Dispose GPU** 释放渲染器，同时保留已显示的预览。验证会加载 WASM，但不会获取 GPU。文件原始字节直接交给 Rust，源码显示内容不会经过解析和重新序列化后用于渲染。
...
`Product checks` 工作流执行干净 npm 安装、公开类型检查和生产构建；它**不代表**浏览器 GPU 执行通过。
...
这里实现第一条 M5-03 消费链路：文件选择、显式初始化、baseColor 预览、尺寸、诊断与释放。公开参数控件、通道切换及导出、最新请求调度、材质验收、registry 发布与 Studio 节点编辑属于后续工作。每次操作期间禁用控件，因此这个最小页面不会排队执行并发渲染请求。
